...
runMain
  ...
  detach = do
    ...
    where
      ...
      normalizeLabel =
        map (\c -> if isAlphaNum c || c `elem` ['_', '-'] then toLower c else '-')
          . takeWhile (/= '\0')
  ...
  if skipIfExists
    then do
      bucketExists <-
          ...
          (readProcess "gsutil" ["ls", bucketName] "" >> return True)
      ...
      _ <-
        callProcess
          ...
          [ "compute",
            ...
            gcpZone,
            "--command",
            "bash ~/myphoto-remote-execute.sh '"
              ++ inputBucket
              ++ "' '"
              ++ outputBucketPath
              ++ "' '"
